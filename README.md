...
**Ground Truth（`benchmark/ground_truth/`）：** 中小企業診断士レベルの正解基準。`cash_flow.jsonl` / `sql.jsonl` / `fraud.jsonl` / `general.jsonl` / `advisor.jsonl`（戦略分析・追加予定）。
...
- → ナレッジベクトルアドバイザー（SWOT・3C・PEST・ABテスト・RFM等）
- → 補助金自動検索 + 士業マッチング
- → 人口動態API連携（e-Stat）
- → 製造トレーサビリティ
- → DPOファインチューニング自動化
