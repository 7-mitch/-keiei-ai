...
| 🔒 **LLM攻撃防御** | AIGIS 336監査項目を活用したプロンプトインジェクション検知・遮断 |
...
### LLM攻撃防御（AIGIS 336監査項目連携）

全ての質問は `check_prompt_security()` で検査してからエージェントに渡します。

```
質問入力
    ↓
① プロンプトインジェクション検知
   「ignore previous instructions」「jailbreak」等 10パターン
    ↓
② 機密情報漏洩防止
   「パスワード」「APIキー」「秘密鍵」等の質問を遮断
    ↓
③ AIGIS 336監査項目でリスクスコア評価
   高リスククエリを監査ログに記録
    ↓
エージェント実行（問題なしの場合のみ）
```

内部利用では**監査ログ記録**、外部公開時は**攻撃を完全遮断**します。
...
環境変数1つで**ローカル ⇔ クラウド**を切り替えられます。

| 環境 | LLM | 用途 |
|---|---|---|
| `development` | Ollama（Qwen3:8b） | ローカル・オンプレ・無料・完全セキュア |
| `production` | Claude API | クラウド・外部公開・デモ用 |

```bash
# ローカル（デフォルト）
ENVIRONMENT=development  # Ollama使用・無料・データ外部送信なし

# クラウド本番
ENVIRONMENT=production   # Claude API使用・外部公開対応
```

長期的にはローカルモデルのファインチューニングにより、
**KEIEI-AI専用モデル**を育てることも可能です。
...
│   ├── app/
│   │   ├── agents/
│   │   │   ├── supervisor.py        # ルーティング＋セキュリティ検査
│   │   │   ├── rag_agent.py         # RAG検索（AIGIS連携）
│   │   │   ├── sql_agent.py         # SQL分析
│   │   │   ├── fraud_agent.py       # 不正検知
│   │   │   ├── project_agent.py     # 工程管理
│   │   │   └── universal_ingest.py  # 多形式RAG投入
│   │   ├── api/
│   │   │   ├── chat.py
│   │   │   ├── projects.py          # 工程管理API
│   │   │   └── ...
