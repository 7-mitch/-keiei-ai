...
**🔐 シークレットファースト：** 機密データは絶対ローカル完結。クラウドは高度判断時のみ・匿名化後に利用。

**🔧 拡張可能：** 環境変数1つでLLMプロバイダーを切り替え。新エージェントの追加が容易。外部API連携はオプション。

**🔄 自律改善：** フィードバック蓄積 → DPO学習。モデルが顧客業務に特化していく。

**✅ 品質保証：** 共通ベースプロンプトで全エージェントの回答品質を統一。ハルシネーション防止・専門家相談誘導を標準装備。LangSmithによる全トレースの可視化・監査。
...
| 判断の迷い・思考の混乱 | 認知整理プロトコルが意思決定困難感を軽減 |
| 最新の法令・市場情報の把握 | Tavily APIで毎朝自動収集・RAGに即時反映 |
...
# DBマイグレーション（初回のみ）
psql $DATABASE_URL -f backend/migrations/migrate_planai.sql
psql $DATABASE_URL -f backend/migrations/002_benchmark.sql
psql $DATABASE_URL -f backend/migrations/003_advisor_cognitive.sql
...
| Web情報収集 | Tavily API連携で金融ニュース・業界別法令（介護・医療・建設・製造・法律・経理）を自動収集・RAG統合。APSchedulerで毎朝6時に自動実行。収集ログはソース名・業種別カラーバッジでUI表示 |

### ナレッジベクトルアドバイザー（実装済み）

経営コンサルタント・データサイエンティストレベルの分析フレームワークをAIが自動適用し、即座に戦略提言を生成します。
**認知整理モード**を内包し、心理的負荷が高い状態の経営者の意思決定をサポートします。

#### 動作モード

| モード | トリガー例 | 内容 |
|---|---|---|
| 戦略分析モード | 「SWOT分析して」「3C分析したい」 | フレームワーク自動選択・提言生成 |
| 認知整理モード | 「頭を整理したい」「迷っている」「決められない」 | 4ステップで認知負荷を軽減 |

#### 認知整理プロトコル（4ステップ）

| ステップ | 操作 | 認知科学的根拠 |
|---|---|---|
| ① 言語化 | 感情・懸念を1〜3文で言葉にする | Affect Labeling（Lieberman et al., 2007） |
| ② 量子化 | 緊急×重要の2軸で最大4チャンクに整理 | チャンキング（Miller, 1956） |
| ③ 可視化 | 今週／今月／保留の3ボックスに仕分け | ワーキングメモリ外部化 |
| ④ フィードバック | 整理度スコア（1〜10）を自己評価 | NASA-TLX代替指標として記録 |

> **研究メモ：** 認知整理モードの対話データ（NASA-TLXスコア・セッション数）は
> `GET /api/feedback/cognitive/timeseries` で時系列取得可能。
> 「AI対話による認知負荷軽減」の仮説検証データとして蓄積される。
...
| 認知整理スコア記録 | NASA-TLXスコアを時系列でDB蓄積・APIで取得可能 |
...
├── 「市場動向は？」                     → Web Agent（Tavily優先）
├── 「工程の進捗は？」                   → Project Agent
├── 「資金繰りは？」「試算表は？」        → CashFlow Agent
├── 「評価コメント」「適性診断」          → HR Agent
├── ファイルアップロード時                → FileAnalysis Agent
├── 「SWOT分析」「3C分析」「ABテスト」   → Advisor Agent（戦略分析モード）
├── 「頭を整理したい」「迷っている」      → Advisor Agent（認知整理モード）
...
共通ベースプロンプト（`base_prompt.py`）で全エージェントに統一品質基準を適用します。

**品質保持ルール：** ハルシネーション防止（不確かな情報は明示）、専門家相談誘導（法律・税務・医療は必ず付記）、モデル品質低下を招く指示への対抗、業種・専門性に応じた動的プロンプト調整。

**エージェント別専門プロンプト：**

| エージェント | 専門知識領域 |
|---|---|
| cash_flow | 財務・インボイス・電帳法 |
...
| advisor | 戦略分析・認知整理・Affect Labeling誘導 |
...
**モデル品質保護：** プロンプトインジェクション防御、品質低下指示（「バカになれ」等）への対抗、ハルシネーション誘発パターンの検知、監査ログへの全記録。
...
**Ground Truth（`benchmark/ground_truth/`）：** 中小企業診断士レベルの正解基準。`cash_flow.jsonl` / `sql.jsonl` / `fraud.jsonl` / `general.jsonl` / `advisor.jsonl`（戦略分析・認知整理）。

**LLM-as-Judge 自動採点（5指標）：** faithfulness / relevancy / completeness / business_value / routing_correct

**DPO学習サイクル：** 👍👎 → DB蓄積 → chosen/rejected生成 → ファインチューニング → モデル自動改善

**認知整理スコアAPI：**
```bash
# NASA-TLXスコアの時系列データ取得（論文データ用）
GET /api/feedback/cognitive/timeseries

# 認知整理モード統計取得
GET /api/feedback/stats  # レスポンスに cognitive_clarity フィールドを含む
```
...
| Tavily API | Web検索・最新情報収集（金融ニュース・業界別法令） |
| APScheduler | 定期自動収集（毎朝6時） |
| Playwright | Webスクレイピング（フォールバック） |
...
│   │   │   ├── hr_agent.py          # 人事・適性診断
│   │   │   └── web_agent.py         # Web情報収集（Tavily優先・業界別法令・定期自動収集）
│   │   ├── api/
│   │   │   ├── auth.py              # 認証
│   │   │   ├── chat.py              # チャット・ファイルアップロード
│   │   │   ├── web.py               # Web収集API（金融ニュース・業界別法令・URL指定）
│   │   │   └── feedback.py          # フィードバック・ベンチマーク・認知整理スコア
│   │   └── core/
│   │       ├── config.py            # 設定管理
│   │       ├── llm_factory.py       # マルチプロバイダーLLMファクトリー
│   │       ├── security.py          # 3層セキュリティ
│   │       └── connection.py        # DB接続管理（web_collection_logsテーブル自動生成）
│   ├── docs_data/                   # 業界別ドキュメント
│   └── migrations/
│       ├── migrate_planai.sql
│       ├── 002_benchmark.sql
│       └── 003_advisor_cognitive.sql  # 認知整理モード対応カラム追加
├── benchmark/
│   ├── ground_truth/
│   │   ├── cash_flow.jsonl
│   │   ├── sql.jsonl
│   │   ├── fraud.jsonl
│   │   ├── general.jsonl
│   │   └── advisor.jsonl            # 戦略分析・認知整理の正解基準
│   └── scripts/
│       └── evaluate.py
├── frontend/
│   └── src/app/
│       └── web/page.tsx             # Web収集UI（業種別カラーバッジ・ソース名表示）
├── docker-compose.yml
└── README.md
```
...
- ✅ Advisor Agent（戦略分析モード + 認知整理モード）
- ✅ NASA-TLXスコア記録・時系列API
- ✅ Web収集エージェント（Tavily API・業界別法令6種・金融ニュース）
- ✅ 定期自動収集（APScheduler・毎朝6時）
- ✅ 収集ログUI（業種別カラーバッジ・ソース名表示）
